...
- 采用**分层目录索引文件**：
  - 根目录有一个 `index.md 或者 README.md`，说明主要的「领域目录」及其用途。
  - 每个领域目录下可以有自己的 `README.md`，说明该目录下有哪些子目录/文件，以及各自用途。
  - 更深一层的子目录也可以继续有 `README.md`，形成多级索引树。
- 知识库根目录约定：
  - 默认认为知识库位于当前项目根目录下的 `docs/` 目录。
  - 如果用户在对话中明确指定了其他路径（例如“我的知识库在 /data/kb”或“用 ./docs 这个目录作为知识库”），则以用户指定的路径作为根目录。
  - 当默认路径 `docs/` 不存在或访问失败时，应向用户确认实际的知识库根目录位置，而不是随意猜测。
- 单个业务文件可能很大：
  - 不要直接用 Read 读取整文件
...
- 默认根目录：否则约定根目录为当前项目下的 `docs/`。
  - 使用 shell 显式检查目录是否存在：优先使用 `test -d docs`，或退而求其次使用 `ls -d docs`。
  - 注意：禁止使用 `Glob "docs" in .` 这类模式来判断目录是否存在，`Glob` 只返回文件路径，不返回目录本身，空结果并不能区分“目录不存在”和“目录存在但为空”。
- 只有在根目录已通过 `test -d` 等方式确认存在时，才使用 Glob 在该目录下检索内容，并把目录作为 `path`，例如：
  - 索引文件：`pattern="**/index.md"`, `path="docs"`
  - 所有 Markdown：`pattern="**/*.md"`, `path="docs"`
- 如果默认 `docs/` 不存在（`test -d` 失败）：不要猜测其他目录，明确告诉用户未找到默认根目录，并让用户指定实际知识库路径。
...
2. 分层查看目录索引 `index.md 或者 README.md`
   - 使用一个「当前工作目录」的概念：
     - 默认从用户指定的知识库根目录开始；如果用户未指定，则使用当前目录。
   - 在当前工作目录下，如果存在 `index.md` 或 `README.md`：
   ...
   - 对于候选子目录：
     - 递归进入该子目录，将其作为新的「当前工作目录」，继续查找其中的 `index.md` 或 `README.md` 并重复上述过程。
...
1. **候选文件选择**
   - 根据 `index.md 或者 README.md` 和文件名、路径判断相关度
...
2. **选择候选 PDF**
   - 根据 `index.md 或者 README.md` 中的描述，选择最相关的 1-3 个文件
...
2. **选择候选 Excel**
   - 根据 `index.md 或者 README.md` 和文件/工作表命名，选择最相关的表
